...
void TLGManager::sendMenu(std::string wellcome, std::string message, std::string command, bool edit){
    if (edit && menu_id!=0) {
...
void TLGManager::sendMessage(std::string message) {
    tlg_client->sendMessage(message.c_str());
}

void TLGManager::setUser(std::string chat_id) {
    tlg_client->setChatID(chat_id.c_str());
}
